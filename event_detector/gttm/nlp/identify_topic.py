...
class TopicIdentifier:
                ...
                if l_code != -1:
                    ...
                    try:
                        ...
                        topic_words = re.sub(r'((?P<brackets>[()])|(?P<number>\-?\d*\.?\d+)|(?P<operator>[+\-\*\/]))',
                                             ' ', topic)
                        topic_words = '{:d}: {}'.format(int(l_code), ' '.join(map(str.strip, topic_words.split())))
                        cluster_topics.append([l_code, best_topic[0], best_topic[1], topic, topic_words])
                    except:
                        print('Error')
                    t_idx = t_idx + 1
        return cluster_topics
    ...
    def save_cluster_topics(self, folder_path, prefix):
        ...
        topics_file_path = folder_path + os.sep + "{}_cluster_topics.csv".format(prefix)
        ...
        if folder_path != "" and prefix != "":
            ...
            with open(topics_file_path, 'w', newline='', encoding='utf-8') as csvFile:
                ...
                writer.writerows([['Cluster Label', 'Topic Code', 'Topic relevance (percent)', 'Topic', 'Topic words']])
...
class LDATopicIndentifier(TopicIdentifier):
    ...
    def identify_topics(self, labels, texts):
        ...
        self.corpus = [dictionary.doc2bow(cleandoc) for cleandoc in self.cleaned]
        self._model = models.ldamodel.LdaModel(self.corpus, num_topics=self.num_topics, id2word=dictionary)
        if self._model is not None:
